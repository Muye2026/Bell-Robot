...
def parse_version(value: str) -> tuple[int, int, str]:
    parts = value.split(".")
    if len(parts) != 2:
        raise argparse.ArgumentTypeError("version must use major.minor format, for example 1.1")
    try:
        major = int(parts[0])
        minor = int(parts[1])
    ...
    if major < 0 or minor < 0:
        raise argparse.ArgumentTypeError("version parts must be non-negative")
...
def write_header(path: Path, weights: list[int], bias: int, version: tuple[int, int, str]) -> None:
    major, minor, version_text = version
...
static constexpr uint32_t kSeatModelVersionMajor = {major};
static constexpr uint32_t kSeatModelVersionMinor = {minor};
static constexpr const char *kSeatModelVersion = "{version_text}";
